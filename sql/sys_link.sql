...
select '链接管理', '0', '1', 'link', 'system/link/index', 1, 0, 'C', '0', '0', 'system:link:list', 'link', 'admin', sysdate(), '', null, '链接管理菜单'
where not exists (
  select 1 from sys_menu where menu_type='C' and component='system/link/index'
);

-- 按钮权限
select @menu_id := (select menu_id from sys_menu where menu_type='C' and component='system/link/index' order by menu_id desc limit 1);

insert into sys_menu (menu_name, parent_id, order_num, path, component, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark)
select '链接查询', @menu_id, '1',  '#', '', 1, 0, 'F', '0', '0', 'system:link:query',        '#', 'admin', sysdate(), '', null, ''
where @menu_id is not null
  and not exists (select 1 from sys_menu where parent_id=@menu_id and menu_type='F' and perms='system:link:query');

insert into sys_menu (menu_name, parent_id, order_num, path, component, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark)
select '链接新增', @menu_id, '2',  '#', '', 1, 0, 'F', '0', '0', 'system:link:add',          '#', 'admin', sysdate(), '', null, ''
where @menu_id is not null
  and not exists (select 1 from sys_menu where parent_id=@menu_id and menu_type='F' and perms='system:link:add');

insert into sys_menu (menu_name, parent_id, order_num, path, component, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark)
select '链接修改', @menu_id, '3',  '#', '', 1, 0, 'F', '0', '0', 'system:link:edit',         '#', 'admin', sysdate(), '', null, ''
where @menu_id is not null
  and not exists (select 1 from sys_menu where parent_id=@menu_id and menu_type='F' and perms='system:link:edit');

insert into sys_menu (menu_name, parent_id, order_num, path, component, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark)
select '链接删除', @menu_id, '4',  '#', '', 1, 0, 'F', '0', '0', 'system:link:remove',       '#', 'admin', sysdate(), '', null, ''
where @menu_id is not null
  and not exists (select 1 from sys_menu where parent_id=@menu_id and menu_type='F' and perms='system:link:remove');

insert into sys_menu (menu_name, parent_id, order_num, path, component, is_frame, is_cache, menu_type, visible, status, perms, icon, create_by, create_time, update_by, update_time, remark)
select '链接导出', @menu_id, '5',  '#', '', 1, 0, 'F', '0', '0', 'system:link:export',       '#', 'admin', sysdate(), '', null, ''
where @menu_id is not null
  and not exists (select 1 from sys_menu where parent_id=@menu_id and menu_type='F' and perms='system:link:export');
